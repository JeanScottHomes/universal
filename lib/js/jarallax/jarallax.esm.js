...
class Jarallax {
	...
	onScroll(force) {
		...
		self.isElementInViewport =
			...
			viewportRect.left <= global$2.innerWidth; // stop calculations if item is not in viewport
...
class VideoWorker {
	...
	getVideo(callback) {
		...
		self.onAPIready(() => {
			...
			if (self.type === 'youtube') {
				...
				self.player =
					self.player || new global$1.YT.Player(self.playerID, self.playerOptions);
			...
			if (self.type === 'vimeo') {
				...
				self.player =
					self.player || new global$1.Vimeo.Player(self.$video, self.playerOptions); // set current time for autoplay
	...
	onAPIready(callback) {
		...
		if (self.type === 'youtube') {
			...
			if (
				(typeof global$1.YT === 'undefined' || global$1.YT.loaded === 0) &&
