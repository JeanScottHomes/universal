...
alert('sitemap test - v24');
    ...
    const targetUrl = "https://www.jeanscotthomes.com/homes-details/";
    let matchingLinks = [];
    let hasAlerted = false;

    function scanShadowRoot() {
      console.log("Scanning shadow root for matching links...");
      const host = document.querySelector('.ihf-container');
      if (host && host.shadowRoot) {
        const shadowContent = host.shadowRoot.querySelectorAll('a[href]');
        shadowContent.forEach(link => {
          if (link.href.includes(targetUrl) && !matchingLinks.includes(link.href)) {
            matchingLinks.push(link.href);
          }
        });

        if (matchingLinks.length > 0 && !hasAlerted) {
          alert(`Found ${matchingLinks.length} matching links:\n\n` + matchingLinks.join("\n"));
          hasAlerted = true;
        } else {
          console.log("No matching links found in shadow root.");
        }
      } else {
        console.log("Shadow root not yet available.");
      }
    }
    ...
    // Optional fallback scan after delay
    setTimeout(scanShadowRoot, 3000);
