<?php 
/*
Project: Featured Listings Add-on (Custom Write Panel)
Description: Creates a custom post type for Featured Listings.
Version: 2.0.0
Author: IDXCentral
...
add_action( 'init', 'idxc_register_post_type_featuredlistings' );

/* Create the post type */
function idxc_register_post_type_featuredlistings() 
{
...
'query_var' => 'featured-listings', 														// False to prevent queries, or string value of the query var to use for this post type
'has_archive' => false, 														// Set to false to disable the archive page
'rewrite' => array('slug' => 'featured-listings', 'with_front' => false), 				// used in the single post type url
...
register_post_type( 'idxc_featlist', $posttype_args ); // Post type. (max. 20 characters, can not contain capital letters or spaces)  
}



function custom_featured_listings_rewrite() {
    add_rewrite_rule(
        '^featured-listings/page/([0-9]+)/?$',
        'index.php?pagename=featured-listings&paged=$matches[1]',
        'top'
    );
}
add_action('init', 'custom_featured_listings_rewrite');
...
// Enable Layout Settings metabox, which includese Custom Body Class for customizations (removed since it was not saving values for some reason)
//add_post_type_support( 'idxc_featlist', 'genesis-layouts' );

// ******************
// Add Taxonomy: Category == Start
$taxonomy_name = 'Category';  					// Singular version
$taxonomy_name_plural = 'Categories';  			// Plural version
$taxonomy_slug = 'listing_category'; 				// Slug for the taxonomy (lowercase and underscore_separated)
...
register_taxonomy( $taxonomy_slug, array( 'idxc_featlist' ), $args );
// Add Taxonomy: Catebory == End 
// ******************

/* Add Metabox (Featured Listings) */ 
add_action( 'cmb2_admin_init', 'idxc_mb_create_featuredlistings' );
/**
 * Hook in and add metabox. Can only happen on the 'cmb2_admin_init' or 'cmb2_init' hook.
 */
function idxc_mb_create_featuredlistings() {
  ...
  $idxc2_metabox = new_cmb2_box( array(
    'id'            => $prefix . 'metabox',
    'title'         => __( 'Featured Listings Information', 'cmb2' ),
    'object_types'  => array( 'idxc_featlist', ), // Post type
    'context'    => 'normal',
    'priority'   => 'high',
  ...
  $idxc2_metabox->add_field( array(
    'name'       => __( 'Sold Date', 'cmb2' ),
    'desc'       => __( 'Enter closing date.', 'cmb2' ),
    'id'         => $prefix . 'date_sold',
    'type'       => 'text_date',
  ) );
  ...
  $idxc2_metabox->add_field( array(
    'name'       => __( 'Sold Price', 'cmb2' ),
    'desc'       => __( 'Enter the price as a number without any symbols or decimal places.', 'cmb2' ),
    'id'         => $prefix . 'price_sold',
    'type'       => 'text_small',
  ) );
  ...
  $idxc2_metabox->add_field( array(
    'name'       => __( 'Zillow 3D Tour', 'cmb2' ),
    'desc'       => __( 'Example: https://www.zillow.com/...', 'cmb2' ),
    'id'         => $prefix . 'zillow_tour_url',
    'type'       => 'text_url',
    'protocols' => array( 'https' ), // Array of allowed protocols
  ) );
  ...
  $idxc2_metabox->add_field( array(
        ...
        'desc'       => __( '', 'cmb2' ),
        'id'         => $prefix . 'category',
        'taxonomy'       => 'listing_category', // Enter Taxonomy Slug
  ...
  $idxc2_metabox->add_field( array(
    'name'       => __( 'Custom body class', 'cmb2' ),
    'desc'       => __( '', 'cmb2' ),
    'id'         => '_custom_body_class',
    'type'       => 'text',
  ) );
...
// Shortcode: Display Featured Listings 
// Requires: Featured Listings add-on (custom post type)
...
//         [mdao_listing_gallery number_of_posts="3"] (see below for other options)
//      		Ready Class(es): 
//				none
...
function mdao_listing_gallery_func($atts, $content = null) {
  ...
  $args = array(
    ...
    'post_type' => array( 'idxc_featlist' ),		// post types allowed
  ...
  if ($order_by == 'price') {
    $args['meta_key'] = '_idxc_mb_featuredlistings_price';  // Custom field for price
  }
  ...
  case 'slider1':
        $layout_include = 'layouts/slider1.php';
        $class .= ''; // Replace with your actual class for this layout
        break;
...
<div id="featured_listings_ao_container" class="ao_fl_featured_listings<?php if ($class != "") { echo " " . $class;} ?>" >
  ...
  <?php if ($title != "") { ?>
    <h2><?php echo $title; ?>title here</h2>
  <?php } ?>
  ...
  <?php global $post; $recent = new WP_Query($args); while($recent->have_posts()) : $recent->the_post();
      ...
      $price_sold = get_post_meta($post->ID, $prefix . 'price_sold', true);
...
<?php if ($layout == "slider1") { // Add script for slider1 layout ?>
<script>
jQuery(document).ready(function($) {
   if ($('.slick_listings_slider1').length) {
     $('.slick_listings_slider1').slick({
       infinite: true,
       slidesToShow: 1,
       slidesToScroll: 1,
       arrows: true,
       prevArrow: '<button type="button hello" class="slick-prev"></button>',
...
add_shortcode("mdao_listing_gallery", "mdao_listing_gallery_func");
...
add_filter( 'cmb2_sanitize_text_small', 'sanitize_price_field', 10, 5 );
function sanitize_price_field( $override_value, $value, $object_id, $field_args, $sanitizer_object ) {
    // Check if the current field is either 'price' or 'price_sold'
    if ( '_idxc_mb_featuredlistings_price' === $field_args['id'] || '_idxc_mb_featuredlistings_price_sold' === $field_args['id'] ) {
